...
export default function AtoresAdmin() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setAtores(await adminService.getAtores());
    } catch {
  ...
  function openEdit(ator) {
    setForm({
      nome: ator.nome ?? "",
      foto: ator.foto ?? "",
      nacionalidade: ator.nacionalidade ?? "",
      data_nascimento: ator.data_nascimento?.split("T")[0] ?? "",
      biografia: ator.biografia ?? "",
    });
    setModal({ mode: "edit", ator });
  }
  ...
  async function handleSave() {
    ...
    try {
      if (modal.mode === "edit") {
        await adminService.updateAtor(modal.ator.id, body);
        showToast("Ator atualizado!");
      } else {
        await adminService.createAtor(body);
        showToast("Ator criado!");
      }
      setModal(null);
      load();
    } catch (e) {
  ...
  async function handleDelete() {
    try {
      await adminService.deleteAtor(deleteTarget.id);
      showToast("Ator excluído!");
  ...
  return (
    <div style={{ position: "relative" }}>
      <AdminHeader
        title="Atores"
        subtitle="Gerenciar elenco"
        action={
          <button style={btnDanger} onClick={openCreate}>+ Novo Ator</button>
        }
      />
              ...
              {atores.length === 0 ? (
                ...
                atores.map((a) => (
                  <tr key={a.id} style={{ borderBottom: "1px solid #2a2a2a" }}>
                    ...
                    <td style={{ ...td, color: "#888", fontSize: "0.85rem" }}>{a.nacionalidade || "—"}</td>
                    ...
                    <td style={{ ...td, width: 90 }}>
                      <button style={btnIconLight} onClick={() => openEdit(a)} title="Editar">✏</button>
                      <button style={btnIconRed} onClick={() => setDeleteTarget(a)} title="Excluir">🗑</button>
                    </td>
                  </tr>
                ))
              )}
      ...
      {modal && (
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setModal(null)}>
          <div style={dialogStyle}>
            <div style={dHeader}>
              <span>{modal.mode === "edit" ? "Editar Ator" : "Novo Ator"}</span>
            ...
            <div style={{ padding: "20px 24px", overflowY: "auto", flex: 1 }}>
              <div style={grid2}>
                <div style={{ gridColumn: "1/-1" }}>
                  ...
                  <input {...field("nome")} placeholder="Nome do ator" />
                </div>
                <div style={{ gridColumn: "1/-1" }}>
